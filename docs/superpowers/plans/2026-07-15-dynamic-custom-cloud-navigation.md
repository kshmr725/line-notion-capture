...
- [ ] Write a failing test that inserts labels for two tenants and asserts only the requested tenant's ordered mapping is returned.
- [ ] Run `.venv/bin/python -m pytest -q tests/portal/test_db.py -k cloud_labels` and confirm RED.
- [ ] Implement one tenant-filtered query against `tenant_clouds`.
- [ ] Run the focused test and commit `feat: expose tenant cloud labels`.
...
- [ ] Write failing tests proving a tenant custom Cloud appears on the homepage, top nav, search selector, and uses its persisted label on item cards.
- [ ] Run the focused tests and confirm RED.
- [ ] Build the catalog once per request from canonical definitions plus safe tenant metadata and indexed unknown keys.
- [ ] Pass the catalog through context, home, search, item card, breadcrumbs, and filter summary helpers.
- [ ] Run focused tests and commit `feat: surface custom clouds across navigation`.
...
- [ ] Write failing tests for a 200 custom page, exact-key item isolation, accessible shell, and 404 for unknown/unowned keys.
- [ ] Run focused tests and confirm RED.
- [ ] Allow only catalog-owned route keys and render a generic custom layout.
- [ ] Hide table/chart/slides/briefing links for custom keys until derived-view schemas exist.
- [ ] Run focused and accessibility tests; commit `feat: add custom cloud workspace`.
...
- [ ] Document dynamic custom Cloud behavior and the projection-only boundary.
- [ ] Run `.venv/bin/python -m pytest -q` and `git diff --check`.
- [ ] Browser-verify home, custom Cloud, search filter, and responsive navigation.
